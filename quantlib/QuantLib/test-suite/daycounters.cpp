...
void DayCounterTest::testActual365NoLeap() {
	BOOST_TEST_MESSAGE("Testing actual/365 (no leap) date day counters...");
	...
	for(Size i=0;i<testStartDates.size();++i) {
		Time calculated = dayCounter.yearFraction(testStartDates[i], testEndDates[i]);
		if (std::fabs(calculated-expected[i]) > 1.0e-12) {
			BOOST_ERROR("from " << testStartDates[i]
			<< " to " << testEndDates[i] << ":\n"
				<< "    calculated: " << calculated << "\n"
...
test_suite* DayCounterTest::suite() {
	...
	suite->add(QUANTLIB_TEST_CASE(&DayCounterTest::testActual365NoLeap));
    return suite;
}
