...
namespace QuantLib {
    ...
    //! parallel shift PV01 sensitivity analysis for a Quote vector
    /*! returns a pair of first and second derivative values calculated as
        ...
        All Quotes are tweaked together in a parallel fashion.
    */
    std::pair<Real, Real>
    parallelAnalysis(const std::vector<Handle<Quote> >&,
    ...
    //! parallel shift PV01 sensitivity analysis for a Quote matrix
    /*! returns a pair of first and second derivative values calculated as
        prescribed by SensitivityAnalysis. Second derivative might not be
        available depending on SensitivityAnalysis value.
    ...
    */
    std::pair<Real, Real>
    parallelAnalysis(const std::vector<std::vector<Handle<Quote> > >&,
                     const std::vector<boost::shared_ptr<Instrument> >&,
                     const std::vector<Real>& quantities,
                     Real shift = 0.0001,
                     SensitivityAnalysis type = Centered,
    ...
    //! (bucket) PV01 sensitivity analysis for a (single) Quote
    /*! returns a pair of first and second derivative values calculated as
        prescribed by SensitivityAnalysis. Second derivative might not be
        available depending on SensitivityAnalysis value.
    ...
    */
    std::pair<Real, Real>
    bucketAnalysis(Handle<Quote> quote,
                   const std::vector<boost::shared_ptr<Instrument> >&,
                   const std::vector<Real>& quantities,
                   Real shift = 0.0001,
                   SensitivityAnalysis type = Centered,
    ...
    //! (bucket) parameters' sensitivity analysis for a (single) Quote
    ...
    */
    ...
    //! bucket PV01 sensitivity analysis for a Quote vector
    /*! returns a pair of first and second derivative vectors calculated as
        ...
        The (bucket) Quotes are tweaked one by one separately.
    */
    ...
    //! bucket parameters' sensitivity analysis for a Quote vector
    /*! returns a vector (one element for each paramet) of pair of first and
    ...
    */
    ...
    //! bucket sensitivity analysis for a Quote matrix
    ...
    */
    ...
    bucketAnalysis(const std::vector<std::vector<Handle<Quote> > >&,
                   const std::vector<boost::shared_ptr<Instrument> >&,
                   const std::vector<Real>& quantities,
                   Real shift = 0.0001,
                   SensitivityAnalysis type = Centered);
