  ...
  public:
    %extend {
        MCAmericanEnginePtr(const GeneralizedBlackScholesProcessPtr& process,
							...
							intOrNull nCalibrationSamples = Null<Size>()) {
            boost::shared_ptr<GeneralizedBlackScholesProcess> bsProcess =
                 boost::dynamic_pointer_cast<GeneralizedBlackScholesProcess>(
                                                                     process);
            QL_REQUIRE(bsProcess, "Black-Scholes process required");
...
typedef boost::shared_ptr<PricingEngine> PerturbativeBarrierOptionEnginePtr;
%}

%rename(PerturbativeBarrierOptionEngine) PerturbativeBarrierOptionEnginePtr;
class PerturbativeBarrierOptionEnginePtr : public boost::shared_ptr<PricingEngine> {
  public:
      %extend {
	      PerturbativeBarrierOptionEnginePtr(
		  const GeneralizedBlackScholesProcessPtr& process,
		  Natural order = 1,
		  ...
		  boost::shared_ptr<GeneralizedBlackScholesProcess> bsProcess =
                 boost::dynamic_pointer_cast<GeneralizedBlackScholesProcess>(
                                                                     process);
		  QL_REQUIRE(bsProcess, "Black-Scholes process required");
		  return new PerturbativeBarrierOptionEnginePtr(
                 new PerturbativeBarrierOptionEngine(bsProcess, order, zeroGamma));
		  }
	  }
};
      ...
      %extend {
	      FdHestonBarrierEnginePtr(  
            ...
            return new FdHestonBarrierEnginePtr(
				new FdHestonBarrierEngine(hModel, tGrid, xGrid, vGrid, dampingSteps, schemeDesc));
		  }
	  }
...
class FdBlackScholesBarrierEnginePtr : public boost::shared_ptr<PricingEngine> {
  public:
      %extend {
	      FdBlackScholesBarrierEnginePtr(
		        const GeneralizedBlackScholesProcessPtr& process,
                Size tGrid = 100, 
				Size xGrid = 100, 
				Size dampingSteps = 0,
			...
			boost::shared_ptr<GeneralizedBlackScholesProcess> bsProcess =
                 boost::dynamic_pointer_cast<GeneralizedBlackScholesProcess>(process);
            QL_REQUIRE(bsProcess, "Black-Scholes process required");
            return new FdBlackScholesBarrierEnginePtr(
                 new FdBlackScholesBarrierEngine(bsProcess, tGrid, xGrid, dampingSteps, schemeDesc, localVol, illegalLocalVolOverwrite));	
			}
	  }
};
...
public:
    ...
    static Real blackFormula(Option::Type optionType,
            Real strike,
            Real forward,
            Real stdDev,
            Real discount = 1.0,
            Real displacement = 0.0) {
                return BlackFormula::blackFormula(optionType, strike, forward, stdDev, discount, displacement);
        }

	static Real blackFormulaImpliedStdDev(Option::Type optionType,
										  Real strike,
			                              Real forward,
			                              Real blackPrice,
			                              Real discount = 1.0,
			...
			return BlackFormula::blackFormulaImpliedStdDev(optionType,
					strike,
					forward,
					blackPrice,
					discount,
	...
	static Real blackFormulaImpliedStdDevRA(Option::Type optionType,
										  Real strike,
			                              Real forward,
			                              Real blackPrice,
			                              Real discount = 1.0,
			                              ...
			                              Real accuracy = 1.0e-6,
			                              Natural maxIterations = 100) {
			return BlackFormula::blackFormulaImpliedStdDevRA(optionType,
					strike,
					forward,
					blackPrice,
					discount,
