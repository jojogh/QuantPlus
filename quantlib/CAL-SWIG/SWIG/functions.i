  ...
  public:
    ...
    Disposable<Array> values(const Array& x) const {
      //  QL_FAIL("Not implemented");
        ...
        for (Size i=0; i<x.size(); i++)
            PyTuple_SetItem(tuple,i,PyFloat_FromDouble(x[i]));
		PyObject* pyFunction = PyObject_GetAttrString(function_,(char*)"values");
        PyObject* pyResult = PyObject_CallObject(pyFunction, tuple);
        Py_XDECREF(tuple);
		Py_XDECREF(pyFunction);
        QL_ENSURE(pyResult != NULL, "failed to call Python function");
		Py_ssize_t tmpSize = PyTuple_GET_SIZE(pyResult);
		Array result(tmpSize);
		for (Py_ssize_t i=0; i<tmpSize;i++)
			result[i] = PyFloat_AsDouble(PyTuple_GetItem(pyResult,i));
        Py_XDECREF(pyResult);
        return result;
