...
%template(Array2DMatrix) std::vector<std::vector<Real> >;
typedef std::vector<std::vector<Real> > Array2DMatrix;
%{
    typedef std::vector<std::vector<Real> > Array2DMatrix;
%}
...
class InverseCumulativeRsg {
  public:
   ...
   %extend {
    // improve performance for large packageof random numbers
    Array2DMatrix batchSimulation(Size samples) const {
      std::vector<std::vector<Real> > res(samples);
      for(Size i=0;i<samples;++i) {
        res[i] = self->nextSequence().value;
      }
      return res;
    }
  }
