...
namespace QuantLib {
	...
	: SwapRateHelper(rate, swapTenor, China(China::IB), fixedFreq, ModifiedFollowing, Actual365Fixed(),
				     shiborIndex, Handle<Quote>(), fwdStart, discountingCurve)
	{}

	//! Rate helper for bootstrapping over China inter-bank repo swap
	IBFRSwapRateHelper::IBFRSwapRateHelper(const Handle<Quote>& rate,
			const Period& swapTenor,
			Frequency payFreq,
			const boost::shared_ptr<RepoChina>& iborIndex,
			const Handle<Quote>& rateSpread,
			const Period& fwdStart,
			const Handle<YieldTermStructure>& discountingCurve)
			:  RelativeDateRateHelper(rate),
				tenor_(swapTenor),
			...
			registerWith(iborIndex_);
			registerWith(rateSpread_);
			registerWith(discountHandle_);
			initializeDates();
	}

	void IBFRSwapRateHelper::initializeDates() {
		
		boost::shared_ptr<PricingEngine> engine(new DiscountingSwapEngine(discountRelinkableHandle_,
			false));

		// To set up a IBFRSwap
		const Calendar floatCalendar = China(China::IB); 
		const Date referenceDate = Settings::instance().evaluationDate();
		const Natural fixingDays = iborIndex_->fixingDays();
		const Date spotDate = floatCalendar.advance(referenceDate,
			fixingDays*Days);
		const Date startDate = spotDate + fwdStart_;

		const Spread rateSpread = rateSpread_.empty() ? 0.0 : rateSpread_->value();

		const boost::shared_ptr<RepoChina> repoIndex
			= boost::dynamic_pointer_cast<RepoChina>(iborIndex_);
		swap_ = boost::shared_ptr<IBFRSwap>(
			new IBFRSwap(RepoCompoundingSwap::Payer, 1.0, startDate, tenor_, Period(payFreq_), 
						 quote()->value(), rateSpread, repoIndex));
		...
		// If forecasting need further date
		Leg floatLeg = swap_->floatingLeg();
		boost::shared_ptr<SubPeriodsCoupon>
			tmp = boost::dynamic_pointer_cast<SubPeriodsCoupon>(floatLeg[floatLeg.size()-1]);

		latestDate_ = std::max(latestDate_, tmp->lastEndValueDate());

	}

	Real IBFRSwapRateHelper::impliedQuote() const {
		QL_REQUIRE(termStructure_ != 0, "term structure not set");
		// we didn't register as observers - force calculation
		swap_->recalculate();
		// weak implementation... to be improved
		static const Spread basisPoint = 1.0e-4;
		Real floatingLegNPV = swap_->floatingLegNPV();
		Real result = -floatingLegNPV/(swap_->fixedLegBPS()/basisPoint);
		return result;
	}

	void IBFRSwapRateHelper::setTermStructure(YieldTermStructure* t) {
		// do not set the relinkable handle as an observer -
		// force recalculation when needed
		bool observer = false;
