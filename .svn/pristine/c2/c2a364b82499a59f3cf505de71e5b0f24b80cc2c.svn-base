...
namespace QuantLib {
    ...
    pair<Real, Real>
    parallelAnalysis(const vector<Handle<Quote> >& quotes,
                     ...
                     Real referenceNpv)
    {
        QL_REQUIRE(!quotes.empty(), "empty Quote vector");
        Size n = quotes.size();
    ...
    bucketAnalysis(const vector<Handle<Quote> >& quotes,
    ...
    bucketAnalysis(std::vector<std::vector<Real> >& deltaMatrix, // result
                   ...
                   const vector<Handle<Quote> >& quotes,
    ...
    bucketAnalysis(const vector<vector<Handle<Quote> > >& quotes,
                   const vector<shared_ptr<Instrument> >& instr,
                   const vector<Real>& quant,
                   Real shift,
                   SensitivityAnalysis type)
    {
        QL_REQUIRE(!quotes.empty(), "empty Quote range");
        Size n = quotes.size();
