...
int main(int, char* [])
{
  ...
  //                      new HullWhite(termStructure,reversionParameter,sigma));
		...
		boost::shared_ptr<ShortRateModel> hw0
			(new CoxIngersollRoss(0.055, 0.1, 0.055, 0.05));
        boost::shared_ptr<PricingEngine> engine0(
                      new TreeCallableFixedRateBondEngine(hw0,gridIntervals, termStructure));
  ...
  //                    new TreeCallableFixedRateBondEngine(hw1,gridIntervals));
		...
		boost::shared_ptr<PricingEngine> engine1(new DiscountingBondEngine(termStructure));
