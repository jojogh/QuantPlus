...
class BlackVarianceSurfacePtr
  ...
  public:
    %extend {
		...
		void setInterpolation(const std::string& traits) {
		
			std::string s = boost::algorithm::to_lower_copy(traits);
			if (s == "bilinear")
                boost::dynamic_pointer_cast<BlackVarianceSurface>(*self)->setInterpolation<Bilinear>();
            else if (s == "bicubic")
				boost::dynamic_pointer_cast<BlackVarianceSurface>(*self)->setInterpolation<Bicubic>();
            else
                QL_FAIL("unknown or unsuitable 2D interpolation type: "+s);
