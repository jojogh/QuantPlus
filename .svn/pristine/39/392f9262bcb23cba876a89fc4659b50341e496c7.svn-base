...
namespace QuantLib {
	...
	Real Bond::optionAdjustedSpread(Real targetCleanPrice,
		                            const Handle<YieldTermStructure>& yieldCurve) const {
		
		RelinkableHandle<YieldTermStructure> tmpCurve(yieldCurve);
		Handle<YieldTermStructure> newCurve(yieldCurve.currentLink());
		
		try {

			Brent solver;
			boost::function<Real(Real)> tmpFunc
				= boost::bind(&Bond::priceErrorFromSpread, this, targetCleanPrice, tmpCurve, newCurve, _1);
			
			Real oas = solver.solve(tmpFunc, 1e-8, 0.0, 0.01);
			tmpCurve.linkTo(newCurve.currentLink());
			return oas;
		}
		catch (...) {
			tmpCurve.linkTo(newCurve.currentLink());
	...
	Real Bond::priceErrorFromSpread(Real targetCleanPrice,
		                            RelinkableHandle<YieldTermStructure>& tmpCurve, 
									const Handle<YieldTermStructure>& newCurve,
								    Spread zSpread)const {
		boost::shared_ptr<Quote> spread(new SimpleQuote(zSpread));
		Handle<Quote> zSpreadQuote(spread);		

		tmpCurve.linkTo(boost::shared_ptr<YieldTermStructure>(new ZeroSpreadedTermStructure(newCurve, zSpreadQuote)));
    ...
    Real Bond::accruedAmount(Date settlement) const {
		...
		QL_REQUIRE(BondFunctions::isTradable(*this, settlement),
			"non tradable at " << settlement <<
			" (maturity being " << maturityDate() << ")");

		return CashFlows::accruedAmount(this->cashflows(),
			false, settlement) *
