...
namespace QuantLib {
    ...
    : arguments_(args) {
		...
		redemptionTimes_.resize(args.redemptionDates.size());
		for(Size i=0; i<redemptionTimes_.size();++i)
			redemptionTimes_[i] = 
			    dayCounter.yearFraction(referenceDate, args.redemptionDates[i]);
    ...
    std::vector<Time> DiscretizedCallableFixedRateBond::mandatoryTimes() const {
		...
		for(i=0;i<redemptionTimes_.size();++i) {
			t = redemptionTimes_[i];
			if (t>=0) {
				times.push_back(t);
			}
		}
	...
	void DiscretizedCallableFixedRateBond::addRedemption(Size i) {
		values_ += arguments_.redemptionAmounts[i];
	} 
