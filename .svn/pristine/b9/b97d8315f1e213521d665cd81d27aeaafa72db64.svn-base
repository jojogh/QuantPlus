...
namespace QuantLibAddin {
	...
	Shibor::Shibor(const boost::shared_ptr<ObjectHandler::ValueObject>& properties, 
		           const std::string& p, 
				   const QuantLib::Handle<QuantLib::YieldTermStructure>& h, 
				   bool permanent) 
	: IborIndex(properties, permanent)
	{
		if (to_upper_copy(p)=="ON")
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
			QuantLib::DailyTenorShibor(h));
		else if (to_upper_copy(p)=="1D")
			QL_FAIL("1D is ambigous: please specify ON");
		else if (to_upper_copy(p)=="1W")
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
			QuantLib::Shibor(1*QuantLib::Weeks, h));
		else {
			QuantLib::Period pp = QuantLib::PeriodParser::parse(p);
			pp.normalize();
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
				QuantLib::Shibor(pp, h));
		}
	}

	RepoChina::RepoChina(const boost::shared_ptr<ObjectHandler::ValueObject>& properties, 
		const std::string& p, 
		const QuantLib::Handle<QuantLib::YieldTermStructure>& h, 
		bool permanent) 
		: IborIndex(properties, permanent)
	{
		if (to_upper_copy(p)=="ON" || to_upper_copy(p)=="1D")
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
			QuantLib::RepoChina(1*QuantLib::Days,h));
		else if (to_upper_copy(p)=="7D")
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
			QuantLib::RepoChina(7*QuantLib::Days, h));
		else if (to_upper_copy(p)=="14D")
			libraryObject_ = shared_ptr<QuantLib::IborIndex>(new
			QuantLib::RepoChina(14*QuantLib::Days, h));
		else {
			QL_FAIL("Repo china tenor is only allowed as ON, 7D or 14D");
		}
	}
