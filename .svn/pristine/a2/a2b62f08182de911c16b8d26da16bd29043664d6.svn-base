	...
	revison = getRevison()
        ...
        for line in lines:
            if line.find('__version__')!=-1:
                finds = finds + 1
                if finds == 1:
                    line = 'const char* __version__    = "0.1.r' + revison + '";\n'
                else:
                    line = "const char* __version__ ;\n"
            newFiles.append(line)
        file.close()
        os.remove('../SWIG/CAL.i')
		
        file = open('../SWIG/CAL.i', 'w')
        file.writelines(newFiles)
        file.close()
		
	file = open('version.dat', 'w')
	file.write(revison)
	file.close()
		
    def finalize_options(self): pass
    def run(self):
        print('Generating Python bindings for CAL...')
        
        swig_dir = os.path.join("..","SWIG")
        os.system('swig -python -c++ -modern ' +
                  '-I%s ' % swig_dir +
                  '-outdir CAL -o CAL/cal_wrap.cpp ' +
                  'CAL.i')
...
class my_build_ext(build_ext):
	...
	self.compiler = get_default_compiler()
        ...
        if compiler == 'msvc':
            ...
            extra_link_args = ['/subsystem:windows', '/machine:X64']
            ...
            ql_compile_args.append('-I' + os.getcwd() + '/CAL/include')
            ql_link_args = \
                os.popen('quantlib-config --libs').read()[:-1].split()
            ql_link_args.append('-L' + os.getcwd() + '/CAL/lib')
            ...
            #extra_compile_args.append('-nostartfiles')
...
if os.name == 'posix':
    ...
    exePath = res
    fileList = []
    if os.path.isfile('CAL/lib/libQuantLib.so'):
        fileList.append('CAL/lib/libQuantLib.so')
    else:
        print('warning: unable to detect CAL/lib/libQuantLib.so')

    if os.path.isfile('CAL/lib/libQuantLib.so.0'):
        fileList.append('CAL/lib/libQuantLib.so.0')
    else:
        print('warning: unable to detect CAL/lib/libQuantLib.so.0')

    if os.path.isfile('CAL/lib/libQuantLib.so.0.0.0'):
        fileList.append('CAL/lib/libQuantLib.so.0.0.0')
    else:
        print('warning: unable to detect CAL/lib/libQuantLib.so.0.0.0')

    tmpDF = [(exePath,fileList)]
else:
    tmpDF = []
...
def readVersionFromFile():
    file = open('version.dat', 'r')
    firstLine = file.readlines()[0]
    version = firstLine.strip()
    file.close()
    return version

setup(name             = "CAL-Python",
      version          = "0.1.r" + readVersionFromFile(),
      description      = "Python bindings for the CAL library",
      long_description = """
...
framework for quantitative finance.
      """,
      author           = "Cheng Li, QuantLib Team",
      author_email     = "cheng.li@datayes.com, quantlib-users@lists.sourceforge.net",
      url              = "http://quantlib.org",
      classifiers      = classifiers,
      py_modules       = ['CAL.__init__','CAL.CAL','CAL.EquityOption', 'CAL.DataAPI', 'CAL.Bond', 'CAL.FunctionTools', 'CAL.Config', 'CAL.PyCAL', 'CAL.Statistic', 'CAL.YieldCurve', 'CAL.DateUtilities', 'CAL.DBConnection'],
      ext_modules      = [Extension("CAL._CAL",
                                    ["CAL/cal_wrap.cpp"])
                         ],
