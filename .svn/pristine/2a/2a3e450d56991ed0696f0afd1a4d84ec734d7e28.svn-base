...
namespace QuantLib {
    ...
    void CallableFixedRateBond::setupArguments(
		...
		const Leg& redemp = redemptions();
        arguments->redemption = redemp.back()->amount();
        arguments->redemptionDate = redemp.back()->date();

        const Leg& cfs = cashflows();

        arguments->couponDates.clear();
        arguments->couponDates.reserve(cfs.size()-redemp.size());
        arguments->couponAmounts.clear();
        arguments->couponAmounts.reserve(cfs.size()-redemp.size());

		arguments->redemptionAmounts.clear();
		arguments->redemptionAmounts.reserve(redemp.size()-1);

        for (Size i=0; i<cfs.size()-1; i++) {
            if (!cfs[i]->hasOccurred(settlement, false)) {
				boost::shared_ptr<Coupon> coupon =
					boost::dynamic_pointer_cast<Coupon>(cfs[i]);
				if (coupon) {
                    arguments->couponDates.push_back(cfs[i]->date());
                    arguments->couponAmounts.push_back(cfs[i]->amount());
				} else {
					arguments->redemptionDates.push_back(cfs[i]->date());
					arguments->redemptionAmounts.push_back(cfs[i]->amount());
				}
            }
        }
	...
	CallableFixedRateBond::CallableFixedRateBond(Natural settlementDays,
		const std::vector<Real>& faceAmounts,
		const Schedule& schedule,
		const std::vector<Rate>& coupons,
		const DayCounter& accrualDayCounter,
		BusinessDayConvention paymentConvention,
		...
		issueDate, putCallSchedule) {
			...
			if (!isZeroCouponBond) {
				...
				Date redemptionDate = calendar_.adjust(maturityDate_,
					paymentConvention);
				setSingleRedemption(faceAmounts[0], redemption, redemptionDate);
			}
