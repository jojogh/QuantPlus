...
void BlackFormulaTest::testBlackImpliedVolRAMethod() {
	BOOST_TEST_MESSAGE("Testing Black implied vol with rational approximation...");
	...
	for(Size i=0;i<LENGTH(d);++i){
		...
		Real strike = forward * exp(d[i] * bpvol * std::sqrt(tte));

		Real callPrem = blackFormula(optionType, strike, forward, stdDev, discount);

		Real impliedBpVol = blackFormulaImpliedStdDevRA(optionType,strike, forward, callPrem, discount, 0.0, 1e-12) / sqrt(tte);
...
void BlackFormulaTest::testBlackImpliedVol() {
	BOOST_TEST_MESSAGE("Testing Black implied vol with Newton-Raphson...");
	...
	for(Size i=0;i<LENGTH(d);++i){
		...
		Real impliedBpVol = blackFormulaImpliedStdDev(optionType,strike, forward, callPrem, discount, 0.0, Null<Real>(), 1e-12) / sqrt(tte);
...
test_suite* BlackFormulaTest::suite() {
	...
	suite->add(QUANTLIB_TEST_CASE(
		&BlackFormulaTest::testBlackImpliedVol));
	suite->add(QUANTLIB_TEST_CASE(
		&BlackFormulaTest::testBlackImpliedVolRAMethod));
