...
namespace QuantLib {
	...
	InterpolatedZeroCurve<T>::InterpolatedZeroCurve(
		...
		Compounding compounding,
		Frequency frequency)
		: ZeroYieldStructure(Settings::instance().evaluationDate(), Calendar(), dc),
		InterpolatedCurve<T>(times, yields, interpolator),
		dates_(std::vector<Date>())
	{
		QL_REQUIRE(this->times_.size() >= T::requiredPoints,
			"not enough input dates given");
		QL_REQUIRE(this->data_.size() == this->times_.size(),
