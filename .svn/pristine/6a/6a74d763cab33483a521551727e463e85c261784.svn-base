...
namespace QuantLib {
    ...
    Issuer::Issuer(const std::vector<std::pair<boost::shared_ptr<DefaultProbKey>,
    ...
    : events_(events) {
        ...
        for(Size i=0; i <eventTypes.size(); i++) {
            boost::shared_ptr<DefaultProbKey> keytmp = boost::make_shared<DefaultProbKey>(eventTypes[i], currencies[i],
                seniorities[i]);
