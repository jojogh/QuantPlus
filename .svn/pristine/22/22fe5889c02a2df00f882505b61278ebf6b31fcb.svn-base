...
namespace {
    ...
    */
    ...
    void SINCS(double X, double& SX, double& CS1)
    {
        /*
          Computes SIN(X), COS(X)^2, with series approx. for |X| near PI/2
        */
        //static double PT, EE;
        static double PT = 1.57079632679489661923132169163975;
        static double EE = POW(( PT - fabs(X) ),2);

        if ( EE < 5e-5 )
            {
                SX = SIGN( 1 - EE*( 1 - EE/12 )/2, X );
                CS1 = EE*( 1 - EE*( 1 - 2*EE/15 )/3 );
            }
        else
            {
                SX = sin(X);
    ...
    double KRNRDT(double A, double B,double(*TVTMFN)(double X, double H1, double H2, double H3, double R23, double RUA, double RUB, double AR, double RUC, int 
NUC),double& ERR1 ){
...
namespace QuantLib {
    ...
    namespace {
        ...
        struct integalpha_adapter {
            boost::shared_ptr<YieldTermStructure> r;
            boost::shared_ptr<YieldTermStructure> q;
            integalpha_adapter(
                    boost::shared_ptr<GeneralizedBlackScholesProcess> process)
            : r(*(process->riskFreeRate())),
        ...
        struct sigmaq_adapter {
            boost::shared_ptr<BlackVolTermStructure> v;
            Real s;
            sigmaq_adapter(
                    boost::shared_ptr<GeneralizedBlackScholesProcess> process)
