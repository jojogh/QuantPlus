...
namespace QuantLibAddin {
    ...
    vector<vector<Real> >
    bucketAnalysis(const vector<QuantLib::Handle<QuantLib::Quote> >& q,
                   ...
                   QuantLib::SensitivityAnalysis type)
    {
        ...
        result = QuantLib::bucketAnalysis(q, instr, quant, shift, type);
		vector<vector<Real> > rtn(result.first.size());

		for(size_t i = 0; i!= rtn.size(); ++i) {
			rtn[i].push_back(result.first[i]);
			rtn[i].push_back(result.second[i]);
		}
