...
namespace QuantLib {
	...
	Real BondFunctions::yieldTermStructurePV01(const Bond& bond,
		...
		const Real basisPoint = 0.0001;

		Real zSpread = basisPoint;

		const Real dirtyPriceUp = CashFlows::npv(bond.cashflows(), d,
						  zSpread, dc, comp, freq,
			              false, settlement) *
			              100.0 / bond.notional(settlement);
		...
		const Real dirtyPriceDown = CashFlows::npv(bond.cashflows(), d,
			zSpread, dc, comp, freq,
			false, settlement) *
			100.0 / bond.notional(settlement);

		return (dirtyPriceUp - dirtyPriceDown) /2.0;
