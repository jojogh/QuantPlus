...
namespace {
	...
	QuantLib::Real bsRationalApproxiamation(QuantLib::Real c,
		QuantLib::Real x) {
		using QuantLib::Real;
		using QuantLib::Size;

		static const Real p1 = -0.969271876255, p2 = 0.097428338274, p3 = 1.750081126685;
		static const Real n[5][5] = {{0.0, -0.068098378725, -0.263473754689, 4.714393825758, 14.749084301452},
		{0.440639436211, -5.792537721792, 3.529944137559, -32.570660102526, 0.0},
		{-5.267481008429, -23.636495876611, 76.398155779133, 0.0, 0.0},
		{-9.020361771283, 41.855161781749, 0.0, 0.0, 0.0},
		{-12.150611865704, 0.0, 0.0, 0.0, 0.0}};
		static const Real m[5][5] = {{0.0, 6.268456292246, 30.068281276567, -11.473184324152, -13.954993561151},
		{-6.284840445036, -11.780036995036, -230.101682610568, 261.950288864225, 0.0},
		{-2.310966989723, 86.127219899668, 20.090690444187, 0.0, 0.0},
		{3.730181294225, -50.117067019539, 0.0, 0.0, 0.0},
		{13.723711519422, 0.0, 0.0, 0.0, 0.0}};

		Real v = 0.0;
		const Real squareRootC = std::sqrt(c);
		v += p1 * x + p2 * squareRootC + p3 * c;

		Real xPowered = 0.0;
		Real numerator  = 0.0;
		Real denominator = 0.0;
		for(Size i=0; i<=4; ++i) {
			xPowered = std::pow(x, int(i));
			for(Size j=0; j<=4 && (i+j) <=4; ++j) {
				if ((i+j) >=1 ) {
					numerator += n[i][j] * xPowered * std::pow(squareRootC, int(j));
					denominator += m[i][j] * xPowered * std::pow(squareRootC, int(j));
				}
			}
		}

		v += numerator / (1.0 + denominator);
...
namespace QuantLib {
                                   ...
                                   Natural maxIterations)
    {
		...
		if(otherOptionPrice < QL_EPSILON)
			otherOptionPrice = QL_EPSILON;
	...
	Real blackFormulaImpliedStdDevRA(Option::Type optionType,
								     Real strike,
		                             Real forward,
		                             Real blackPrice,
		                             Real discount,
		...
		forward = forward + displacement;
		strike = strike + displacement;
		Real oldPrice = blackPrice;

		if (optionType == Option::Put) {
			blackPrice = discount * (forward- strike) + blackPrice; 
			optionType = Option::Call;
		}

		blackPrice = std::max(QL_EPSILON, blackPrice);

		const Real normalizedBlackPrice = blackPrice / forward / discount;
		const Real moneyness = std::log(forward / strike);

		Real stdDev = bsRationalApproxiamation(normalizedBlackPrice, moneyness);

		if (stdDev <= 0.0)
			stdDev = 1.0;

		//Do some refinement on the result
		BlackImpliedStdDevHelper f(optionType, strike, forward,
			blackPrice/discount);
		NewtonSafe solver;
		solver.setMaxEvaluations(maxIterations);
		Real minSdtDev = 0.0, maxStdDev = 24.0; // 24 = 300% * sqrt(60)
		stdDev = solver.solve(f, accuracy, stdDev, minSdtDev, maxStdDev);
		QL_ENSURE(stdDev>=0.0,
			"stdDev (" << stdDev << ") must be non-negative");
		return stdDev;
	}

	Real blackFormulaImpliedStdDevRA(const boost::shared_ptr<PlainVanillaPayoff>& payoff,
									Real strike,
									Real forward,
									Real blackPrice,
									Real discount,
									...
									Real accuracy,
									Natural maxIterations) {
       return blackFormulaImpliedStdDevRA(payoff->optionType(), payoff->strike(),
						forward, blackPrice, discount, displacement,  accuracy, maxIterations);
	}
